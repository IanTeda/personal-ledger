//! # Backend Configuration Types and Defaults
//!
//! This module defines [`BackendConfig`] and related constants for configuring the
...
pub const DEFAULT_BACKEND_ADDRESS: &str = "127.0.0.1";
...
pub const DEFAULT_BACKEND_PORT: u16 = 50059;
...
pub const DEFAULT_BACKEND_TLS_ENABLED: bool = false;
...
pub const DEFAULT_BACKEND_TLS_CERT_PATH: Option<&str> = None;
...
pub const DEFAULT_BACKEND_TLS_KEY_PATH: Option<&str> = None;
...
pub const DEFAULT_BACKEND_DATA_DIR: Option<&str> = Some("data/");
...
pub const DEFAULT_BACKEND_DATABASE_PATH: Option<&str> = Some("personal_ledger.sqlite");


#[derive(Debug, Clone, serde::Deserialize)]
/// Server-specific configuration values for the Personal Ledger backend.
///
/// `BackendConfig` holds the host/port the gRPC server will bind to, the SQLite database path,
...
///
/// ```rust
/// use personal_ledger_backend::config::BackendConfig;
...
pub struct BackendConfig {
    ...
    /// Optional configuration directory path where config files are located.
    /// When `None`, defaults to `config/` in the current working directory.
    /// This directory is used to store configuration files for the application.
    /// It is recommended to use a dedicated config directory to avoid clutter.
...
impl Default for BackendConfig {
    ...
    ///
    /// Returns a `BackendConfig` with sensible defaults:
    ...
    fn default() -> Self {
        Self {
            address: DEFAULT_BACKEND_ADDRESS.to_string(),
            port: DEFAULT_BACKEND_PORT,
            data_dir: DEFAULT_BACKEND_DATA_DIR.map(PathBuf::from),
            log_level: Some(DEFAULT_LOG_LEVEL),
            tls_enabled: DEFAULT_BACKEND_TLS_ENABLED,
            tls_cert_path: DEFAULT_BACKEND_TLS_CERT_PATH.map(PathBuf::from),
            tls_key_path: DEFAULT_BACKEND_TLS_KEY_PATH.map(PathBuf::from),
            database_path: DEFAULT_BACKEND_DATABASE_PATH.map(PathBuf::from),
        }
    }
}
...
mod tests {
    ...
    fn default_server_config_values() {
        let s = BackendConfig::default();
        assert_eq!(s.address, DEFAULT_BACKEND_ADDRESS);
        assert_eq!(s.port, DEFAULT_BACKEND_PORT);
        assert_eq!(s.tls_enabled, DEFAULT_BACKEND_TLS_ENABLED);
    ...
    fn deserialize_from_configlib_with_defaults() {
        let cfglib = ConfigLib::builder()
            .set_default("address", DEFAULT_BACKEND_ADDRESS)
            .unwrap()
            .set_default("port", DEFAULT_BACKEND_PORT)
            .unwrap()
            .set_default("tls_enabled", DEFAULT_BACKEND_TLS_ENABLED)
            .unwrap()
        ...
        let server_cfg: BackendConfig = cfglib.try_deserialize().expect("should deserialize");
        assert_eq!(server_cfg.address, DEFAULT_BACKEND_ADDRESS);
        assert_eq!(server_cfg.port, DEFAULT_BACKEND_PORT);
        assert_eq!(server_cfg.tls_enabled, DEFAULT_BACKEND_TLS_ENABLED);
